...
WHERE EXISTS (SELECT 1 FROM shippers, orders WHERE shippers.company_name='United Package'
			  AND customers.customer_id=orders.customer_id AND orders.ship_via=shippers.shipper_id
...
WHERE EXISTS (SELECT 1 FROM categories WHERE products.discontinued=0 AND products.units_in_stock<25
